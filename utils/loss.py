...
def log_cosh_loss(labels, preds):
    loss = torch.log(torch.cosh(preds - labels))
    return torch.mean(loss)


def huber_loss(labels, preds, delta=1.0):
...
def quantile_loss(labels, preds, delta=0.25):
...
def masked_mape_torch(labels, preds, null_val=np.nan, eps=0):
    labels[torch.abs(labels) < 1e-4] = 0
...
def masked_mae_np(labels, preds, null_val=np.nan):
    with np.errstate(divide='ignore', invalid='ignore'):
        if np.isnan(null_val):
            mask = ~np.isnan(labels)
        else:
...
def r2_score_np(labels, preds):
    preds = preds.flatten()
    labels = labels.flatten()
    return r2_score(labels, preds)


def explained_variance_score_np(labels, preds):
    preds = preds.flatten()
    labels = labels.flatten()
    return explained_variance_score(labels, preds)
