...
class gwnet(nn.Module):
    def __init__(self, num_nodes, dropout=0.3, supports=None, gcn_bool=True, addaptadj=True, adjinit=None,
        ...
        self.register_buffer(
            "supports",
            torch.zeros(len(supports), supports[0].shape[0], supports[0].shape[1])
        )
        for i in range(len(supports)):
            self.supports[i] = supports[i]

        receptive_field = in_dim - 1
    ...
    def forward(self, input):
        ...
        if self.gcn_bool and self.addaptadj and self.supports is not None:
            ...
            new_supports = torch.cat((self.supports, adp), dim=0)
