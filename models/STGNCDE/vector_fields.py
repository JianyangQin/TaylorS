...
class VectorField_g(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers, num_nodes, cheb_k, embed_dim,
        ...
        if self.g_type == 'agc':
            ...
            self.residual_pool = nn.Parameter(torch.FloatTensor(embed_dim, hidden_hidden_channels))
    ...
    def agc(self, z):
        ...
        residual = torch.matmul(self.node_embeddings, self.residual_pool)                       #N, dim_out
        x_g = torch.einsum("knm,bmc->bknc", supports, z)      #B, cheb_k, N, dim_in
        x_g = x_g.permute(0, 2, 1, 3)  # B, N, cheb_k, dim_in
        z = torch.einsum('bnki,nkio->bno', x_g, weights) + residual     #b, N, dim_out
        return z
