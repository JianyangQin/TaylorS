...
class cheb_conv_withSAt(nn.Module):
    ...
    def __init__(self, K, cheb_polynomials, in_channels, out_channels):
        ...
        self.register_buffer(
            "cheb_polynomials",
            torch.zeros(len(cheb_polynomials), cheb_polynomials[0].shape[0], cheb_polynomials[0].shape[1])
        )
        for i in range(len(cheb_polynomials)):
            self.cheb_polynomials[i] = cheb_polynomials[i]
...
class cheb_conv(nn.Module):
    ...
    def __init__(self, K, cheb_polynomials, in_channels, out_channels):
        ...
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.Theta = nn.Parameter(K, torch.FloatTensor(in_channels, out_channels))
...
class ASTGCN(nn.Module):
    ...
    def __init__(self, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_for_predict, len_input, num_of_vertices):
        ...
        # self.final_conv = nn.Conv2d(int(len_input/time_strides), num_for_predict, kernel_size=(1, nb_time_filter - in_channels + 1))

        self.final_conv = nn.Conv2d(int(len_input / time_strides), in_channels * num_for_predict, kernel_size=(1, nb_time_filter))
    ...
    def forward(self, x):
        ...
        output = output[:, :, :, -1].permute(0, 2, 1).reshape(-1, self.num_of_vertices, self.in_channels, self.num_for_predict)
