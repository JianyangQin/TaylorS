...
class D2STGNN(nn.Module):
    def __init__(self, **model_args):
        ...
        self._time_per_day  = model_args['time_per_day']
        ...
        # time embedding
        self.T_i_D_emb  = nn.Parameter(torch.empty(self._time_per_day, model_args['time_emb_dim']))
        ...
        self.out_fc_2   = nn.Linear(self._output_hidden, model_args['gap'] * self._in_feat)
    ...
    def _prepare_inputs(self, history_data):
        ...
        # time slot embedding
        time_in_day_feat = self.T_i_D_emb[(history_data[:, :, :, num_feat] * self._time_per_day).type(torch.LongTensor)]    # [B, L, N, d]
    ...
    def forward(self, history_data):
        ...
        forecast    = forecast.transpose(1,2).contiguous().view(forecast.shape[0], forecast.shape[2], self._in_feat, -1)
