...
class STFGNN(nn.Module):
    def __init__(self, config, adj):
        ...
        num_of_vertices = self.config.num_of_vertices
        in_dim = self.config.in_dim
        out_dim = self.config.out_dim
        hidden_dims = self.config.hidden_dims
        first_layer_embedding_size = self.config.first_layer_embedding_size
        out_layer_dim = self.config.out_layer_dim
        activation = self.config.activation
        use_mask = self.config.use_mask
        temporal_emb = self.config.temporal_emb
        spatial_emb = self.config.spatial_emb
        seq_emb = self.config.seq_emb
        seq_emb_dim = self.config.seq_emb_dim
        horizon = self.config.horizon
        strides = self.config.strides

        self.register_buffer("adj",torch.zeros_like(adj))
        self.adj = adj
        ...
        self.Seq_FC = None
        if seq_emb is True:
            self.Seq_FC = nn.Linear(history, seq_emb_dim, bias=True)
            history = seq_emb_dim
