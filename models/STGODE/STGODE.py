...
class GCN(nn.Module):
    def __init__(self, A_hat, in_channels, out_channels,):
        ...
        self.register_buffer("A_hat", torch.zeros_like(A_hat))
        self.A_hat = A_hat
...
class STGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_steps, num_nodes, A_hat):
        """
        Args:
            ...
            num_steps: Number of step in sequeence
        ...
        self.odeg = ODEG(out_channels[-1], num_steps, A_hat, time=6)
...
class STGODE(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input,
        ...
        self.sp_blocks = nn.ModuleList(
            [nn.Sequential(
                ...
                num_steps=num_timesteps_input, num_nodes=num_nodes, A_hat=A_sp_hat),
                STGCNBlock(in_channels=64, out_channels=[64, 32, 64],
                num_steps=num_timesteps_input, num_nodes=num_nodes, A_hat=A_sp_hat)) for _ in range(3)
            ])
        # semantic graph
        self.se_blocks = nn.ModuleList([nn.Sequential(
                STGCNBlock(in_channels=num_features, out_channels=[64, 32, 64],
                num_steps=num_timesteps_input, num_nodes=num_nodes, A_hat=A_se_hat),
                STGCNBlock(in_channels=64, out_channels=[64, 32, 64],
                num_steps=num_timesteps_input, num_nodes=num_nodes, A_hat=A_se_hat)) for _ in range(3)
            ]) 
    ...
    def forward(self, x):
        ...
        return self.pred(x).permute(0, 1, 3, 2)
