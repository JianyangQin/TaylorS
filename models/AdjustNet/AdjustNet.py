...
class AdjustNet_withASTGCN(nn.Module):
    def __init__(self, configs):
        ...
        self.transform_residual = nn.ModuleList([TransformAttention(
            hidden_dim=self.hidden_dim,
            num_of_head=self.num_of_head,
            bn_decay=self.bn_decay
        ) for _ in range(1, self.num_of_layer+1)])

        self.decoder_residual = nn.ModuleList([ASTGCN(
            in_channels=self.hidden_dim,
            K=3,
            nb_chev_filter=self.output_dim,
            nb_time_filter=self.output_dim,
        ...
        self.fc_residual = nn.ModuleList([nn.Conv2d(
        ...
        self.mu_logvar_residual = nn.ModuleList([nn.Sequential(
    ...
    def _get_mu_logvar_residual(self, x, idx):
        out = self.mu_logvar_residual[idx](x)
        mu, log_var = torch.split(out, [self.mu_dim, self.mu_dim], dim=-1)
        return mu, log_var
    ...
    def forward(self, batch):
        taylor_residual, taylor_dev, taylor_dev_mu, taylor_dev_logvar, taylor_residual_mu, taylor_residual_logvar \
            = [], [], [], [], [], []
        ...
        for layer_idx in range(self.num_of_layer):
            ...
            residual_z = self.transform_residual[layer_idx](dev_z, dt_pred, t_pred)
            residual_z = self.decoder_residual[layer_idx](residual_z + t_pred)
            residual = self.fc_residual[layer_idx](residual_z.permute(0, 3, 1, 2))
            residual = residual[:, :, :, -1].permute(0, 2, 1).reshape(-1, self.num_of_vertices, self.input_dim, self.num_of_predict)
            residual_mu, residual_logvar = self._get_mu_logvar_residual(residual, layer_idx)

            taylor_residual.append(residual)
            taylor_dev.append(dev)
            taylor_dev_mu.append(dev_mu)
            taylor_dev_logvar.append(dev_logvar)
            taylor_residual_mu.append(residual_mu)
            taylor_residual_logvar.append(residual_logvar)

        taylor_residual = torch.sum(torch.stack(taylor_residual), dim=0)

        return taylor_residual, taylor_dev, taylor_dev_mu, taylor_dev_logvar, taylor_residual_mu, taylor_residual_logvar
...
class AdjustNet_withGWNET(nn.Module):
    def __init__(self, configs):
        ...
        self.decoder_residual = nn.ModuleList([GWNET(
            num_nodes=configs.num_nodes,
            dropout=configs.dropout,
            supports=configs.supports,
            gcn_bool=configs.gcn_bool,
    ...
    def forward(self, batch):
        ...
        for layer_idx in range(self.num_of_layer):
            ...
            residual_z = self.transform_residual[layer_idx](dev_z, dt_pred, t_pred)
            residual_z = self.decoder_residual[layer_idx](torch.cat((residual_z + t_pred, tid_pred), dim=2))
            residual = self.fc_residual[layer_idx](residual_z.permute(0, 3, 1, 2))
            residual = residual[:, :, :, -1].permute(0, 2, 1).reshape(-1, self.num_of_vertices, self.input_dim, self.num_of_predict)
            residual_mu, residual_logvar = self._get_mu_logvar_residual(residual, layer_idx)
            ...
            # taylor_residual.append(residual / factorial)
            taylor_residual.append(residual)
            taylor_dev.append(dev)
            taylor_dev_mu.append(dev_mu)
            taylor_dev_logvar.append(dev_logvar)
