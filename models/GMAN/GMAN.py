...
class GMAN(nn.Module):
    ...
    def __init__(self, configs):
        ...
        K = configs.K
        d = configs.d
        D = configs.K * configs.d
        input_dim = configs.input_dim
        bn_decay = configs.bn_decay
        self.num_his = configs.num_of_history
        ...
        self.start_conv = FC(input_dims=[input_dim, D], units=[D, D], activations=[F.relu, None],
                       bn_decay=bn_decay)
        self.end_conv = FC(input_dims=[D, D], units=[D, input_dim], activations=[F.relu, None],
                       bn_decay=bn_decay)
        self.register_buffer("SE", configs.SE)
        # self.SE = configs.SE
    ...
    def forward(self, batch):
        ...
        X = self.start_conv(X)
        # STE
        STE = self.STEmbedding(self.SE.to(X.device), TE)
