...
class loss_function(nn.Module):
    ...
    def forward(self, true, pred):
        guide_pred, base_pred, _, dev, mu0, logvar0, mu1, logvar1 = pred

        loss_guide = self.criterion(true, guide_pred)
        ...
        loss = (1 - self.alpha) * (loss_guide + loss_dev) + self.alpha * (loss_base + loss_kld)
        # loss = (1 - self.alpha) * (loss_guide + loss_dev) + self.alpha * (loss_base)

        if torch.sum(torch.isnan(loss)):
            print("loss is nan")

        return loss, loss_guide, loss_base