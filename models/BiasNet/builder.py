...
class build_biasnet_astgcn(build_model):
    def __init__(self, network, train_phase, checkpoint_filename, **args):
        super(build_biasnet_astgcn, self).__init__(network, train_phase, checkpoint_filename, **args)
        ...
        # training parameters
        self.criterion_type = args['guide_train_args']['criterion_type']
        self.optimization = args['guide_train_args']['optimization']

        self.lr = args['guide_train_args']['lr']
        self.weight_decay = args['guide_train_args']['weight_decay']
        self.epsilon = args['guide_train_args']['epsilon']

        self.lr_decay_type = args['guide_train_args']['lr_decay']
        self.lr_decay_step = args['guide_train_args']['lr_decay_step']
        self.lr_decay_rate = args['guide_train_args']['lr_decay_ratio']

        self.clip = args['guide_train_args']['clip']
    ...
    def _get_model_parameter(self):
        ...
        configs.cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor)
                                   for i in cheb_polynomial(L_tilde, self.args['guide_model_args']['K'])]
        ...
        configs.num_of_layer = self.args['guide_model_args']['num_of_layer']
        ...
        # configs.freq = self.args['guide_model_args']['freq']
        configs.input_dim = self.args['data_args']['num_of_features']
        configs.hidden_dim = self.args['guide_model_args']['hidden_dim']
        configs.output_dim = self.args['guide_model_args']['output_dim']
        configs.mu_dim = self.args['guide_model_args']['mu_dim']
        configs.num_of_head = self.args['guide_model_args']['num_of_head']
        configs.bn_decay = self.args['guide_model_args']['bn_decay']
        configs.K = self.args['guide_model_args']['K']
        return configs
...
class build_biasnet_gwnet(build_model):
    def __init__(self, network, train_phase, checkpoint_filename, **args):
        super(build_biasnet_gwnet, self).__init__(network, train_phase, checkpoint_filename, **args)
        self.device = args['device']
        self.device_ids = args['device_ids']
        self.use_multi_gpu = args['use_multi_gpu']
    ...
    def _get_model_parameter(self):
        ...
        configs.dropout = self.args['guide_model_args']['dropout']
        configs.residual_channels = self.args['guide_model_args']['nhid']
        configs.dilation_channels = self.args['guide_model_args']['nhid']
        configs.skip_channels = self.args['guide_model_args']['nhid'] * 8
        configs.end_channels = self.args['guide_model_args']['nhid'] * 16

        configs.adjtype = self.args['guide_model_args']['adjtype']
        configs.gcn_bool = self.args['guide_model_args']['gcn_bool']
        configs.aptonly = self.args['guide_model_args']['aptonly']
        configs.addaptadj = self.args['guide_model_args']['addaptadj']
        configs.randomadj = self.args['guide_model_args']['randomadj']
